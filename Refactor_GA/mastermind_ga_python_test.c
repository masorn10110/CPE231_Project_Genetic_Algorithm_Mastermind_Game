...
int main()
{
    ...
    scanf("%f", &PERMUTATION_RATE);
    scanf("%f", &INVERSION_RATE);
    scanf("%d", &WHITE_WEIGHT);
    scanf("%d", &BLACK_WEIGHT);
    Code answer;
    Guesses guesses;
    guesses.turn = 0;
    guesses.guess = (Guess *)malloc(sizeof(Guess) * 100);
    answer.code = (int *)malloc(sizeof(int) * SLOT);
    int *first_guess = (int *)malloc(sizeof(int) * SLOT);
    // Scanf the first guess
    for (int i = 0; i < SLOT; i++)
    {
        scanf("%d", &first_guess[i]);
    }

    // scanf the answer
    for (int i = 0; i < SLOT; i++)
    {
        scanf("%d", &answer.code[i]);
    }
    ...
    while (guesses.guess[guesses.turn - 1].score.black != SLOT)
    {
        ...
        while (chosen_one_next_guess.size > 1)
        {
            int fitness_min = 0;
            for (int i = 0; i < chosen_one.size; i++)
            {
